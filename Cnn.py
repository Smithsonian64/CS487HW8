...
class cnn(object):
    ...
    def build(self):
        ...
        tf_x_image = tf.reshape(tf_x, shape=[200, 28, 28, 1],
        ...
        h1_pool = tf.compat.v1.layers.max_pooling2d(h1,
                                          ...
                                          strides=(2, 2))
        ## 2n layer: Conv_2
        h2 = tf.compat.v1.layers.conv2d(h1_pool, kernel_size=(3, 3),
                              filters=64,
                              activation=tf.nn.relu)
        ## MaxPooling
        ...
        predictions = {
            'probabilities': tf.nn.softmax(h4,
                                           name='probabilities'),
            'labels': tf.cast(tf.argmax(input=h4, axis=1),
                              tf.int32, name='labels')
        }
        ## Loss Function and Optimization
        cross_entropy_loss = tf.reduce_mean(
            input_tensor=tf.nn.softmax_cross_entropy_with_logits(
                logits=h4, labels=tf.stop_gradient(tf_y_onehot)),
